...
async def delete_password_controller(id: str, email: str, label: PasswordLabelRequest) -> PasswordsResponseModel:
    ...
    users_deleted_passwords.insert_one(pass_rec)
...
async def restore_password_controller(id: str, email: str):
    if not email:
        return {"message": AuthErrorMessages.LOGIN_EXPIRED.value, "status": status.HTTP_401_UNAUTHORIZED}
    ...
    pass_rec = users_deleted_passwords.find_one({"password_id": ObjectId(password_object_id), "owner_email": email})

    if not pass_rec:
        return {"message": PasswordErrorMessages.PASSWORD_NOT_FOUND.value, "status": status.HTTP_404_NOT_FOUND}
    
    users_password_collection.insert_one(pass_rec)
    res = users_deleted_passwords.delete_one({"password_id": password_object_id, "owner_email": email})
...
async def delete_password_forever_controller(id: str, email: str, label: PasswordLabelRequest) -> PasswordsResponseModel:
    if not email:
        return {"message": AuthErrorMessages.LOGIN_EXPIRED.value, "status": status.HTTP_401_UNAUTHORIZED}
